  ...
  useEffect(() => {
    const run = async () => {
      const { data, error } = await supabase
        .from('elements')
        .select('id, system_name, subsystems(id,name)')
      if (error) { console.error(error); setLoading(false); return }

      const map = new Map<string, { subs: Map<string,{id:string,name:string}>, count:number }>()

      for (const r of (data ?? [])) {
        const sys = (r as any).system_name ?? '—'
        const ss = (r as any).subsystems
        if (!map.has(sys)) map.set(sys, { subs:new Map(), count:0 })
        const slot = map.get(sys)!
        slot.count++
        if (ss?.id) slot.subs.set(ss.id, { id:ss.id, name:ss.name })
      }
      const rows_: Row[] = Array.from(map.entries()).map(([sys, val]) => ({
        system_name: sys, subsystems: Array.from(val.subs.values()), count: val.count
        ...
        {rows.map((r,idx)=>(
          <div key={idx} style={{ border:'1px solid #e5e7eb', borderRadius:12, padding:12 }}>
            ...
            <ul style={{ listStyle:'none', padding:0, margin:0, display:'grid', gap:6 }}>
              {r.subsystems.map(ss=>(
                <li key={ss.id}>
                  <a href={`/systems/${encodeURIComponent(r.system_name ?? '-') }?ss=${ss.id}`}

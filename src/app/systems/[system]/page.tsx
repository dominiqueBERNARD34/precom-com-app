  ...
  useEffect(() => {
    const run = async () => {
      const { data, error } = await supabase
        ...
        .eq('system_name', system === '-' ? null : system)
      ...
      for (const r of (data ?? [])) {
        ...
        if (!ss?.id) continue
        if (!map.has(ss.id)) map.set(ss.id, { id:ss.id, name:ss.name, count:0 })
        map.get(ss.id)!.count++
      }
      setSsList(Array.from(map.values()).sort((a,b)=> a.name.localeCompare(b.name)))
      setLoading(false)
    }
    run()
  }, [system])
  ...
  return (
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))', gap:12 }}>
        {ssList.map(ss=>(
          <a key={ss.id} href={`/subsystems/${ss.id}`} style={{
            border:'1px solid #e5e7eb', borderRadius:12, padding:12,
            textDecoration:'none', color:'#0f172a',
            outline: highlight===ss.id ? '2px solid #22c55e' : 'none'
          }}>

...
export default function SubsystemPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const run = async () => {
      const { data: s1 } = await supabase.from('subsystems').select('id,name').eq('id', ssId).single()
      setSs(s1 as any)

      const { data: els } = await supabase
        .from('elements')
        .select('id, physical_tag, denomination, system_name')
        .eq('subsystem_id', ssId)
        .order('physical_tag', { ascending: true })
  ...
  const tabButton = (key:'pre'|'com'|'res', label:string) => (
    <button onClick={()=>setTab(key)}
      style={{
        padding:'8px 12px', border:'1px solid #e5e7eb', background: tab===key ? '#e2e8f0':'#fff',
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:12 }}>
        {tabButton('pre', 'Pré-commissioning')}
        {tabButton('com', 'Commissioning')}
        {tabButton('res', 'Réserves')}
      </div>

      {tab === 'pre' && (
        <div style={{ display:'grid', gap:12 }}>
          <div style={{ border:'1px solid #e5e7eb', borderRadius:12, padding:12 }}>
            <h3 style={{ margin:'4px 0 8px' }}>Contrôle conformité</h3>
          </div>
          <div style={{ border:'1px solid #e5e7eb', borderRadius:12, padding:12 }}>
            <h3 style={{ margin:'4px 0 8px' }}>Essais statiques</h3>
          </div>
      ...
      {tab === 'com' && (
        <div style={{ display:'grid', gap:12 }}>
          <div style={{ border:'1px solid #e5e7eb', borderRadius:12, padding:12 }}>
            <h3 style={{ margin:'4px 0 8px' }}>Tests fonctionnels</h3>
          </div>
          <div style={{ border:'1px solid #e5e7eb', borderRadius:12, padding:12 }}>
            <h3 style={{ margin:'4px 0 8px' }}>Mise en service opérationnel</h3>
          </div>
          <h3>Éléments concernés</h3>
          {commonPanel}
        </div>
      ...
      {tab === 'res' && (
        <div style={{ display:'grid', gap:12 }}>
          <div style={{ border:'1px solid #e5e7eb', borderRadius:12, padding:12 }}>
            <h3 style={{ margin:'4px 0 8px' }}>Fiches réserves</h3>
            <p style={{ color:'#64748b' }}>
              Les réserves sont liées à une fiche de test (anomalie NOK).
            </p>
          </div>
        </div>
      )}
